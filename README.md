...
All tests were written and run successfully with **pytest** (Python 3.8).  
The test suite covers basic, edge, and error cases for `longest_palindromic_substring()`.

### Summary

- Handles odd and even-length palindromes
- Works with empty and single-character strings
- Includes long and identical-character inputs
- Raises `TypeError` for invalid (non-string) input
